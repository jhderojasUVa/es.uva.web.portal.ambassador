...
<ul class="breadcrumb_biblioteca" role="breadcrumb">
	<li class="start"><a href="<cms:link>/</cms:link>">Inicio</a></li>
	<c:forEach items="${nav.items}" var="elem" varStatus="status">
		...
		<c:if test="${not cms:vfs(pageContext).exists[elemento]}" >
			...
			<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb" aria-label="${elem.title}" class="final"><span itemprop="title">${elem.title}</span></li>
			</c:if>
			<c:if test="${!empty navText}">
			<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb" aria-label="${elem.title}" class="final"><span itemprop="title">${navText}</span></li>
			</c:if>
		</c:if>
	</c:forEach>
</ul>
